...
namespace RoadCrossing
{
	...
	public class RCGGameController : MonoBehaviour
	{
		...
		private float inputCooldown = 0.1f; // Delay antar input
		private float lastInputTime = 0f;   // Waktu terakhir input diterima
		...
		void Update()
		{
			...
			else
			{
				...
				if (swipeControls == true && Time.time - lastInputTime > inputCooldown)
				{
					...
					foreach (Touch touch in Input.touches)
					{
						...
						if (touch.phase == TouchPhase.Ended && swipeTimeoutCount > 0)
						{
							...
							else if ((swipeEnd - swipeStart).magnitude < swipeDistance) // Tap Detected
							{
								MovePlayer("forward");
								lastInputTime = Time.time;
							}
